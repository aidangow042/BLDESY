/**
 * PortalPage (tradie screens) — the page frame for the portal job feeds,
 * applications, messages, edit profile and job detail.
 *
 * Same contract as the shell agent's `components/portal/portal-page.tsx`
 * (`title`, `onRefresh`, `refreshing`; web `px-4 py-6` + `space-y-6`, h1
 * `text-2xl font-bold`) plus what list-based screens need: `subtitle` (the web
 * `p.text-sm.text-text-secondary` under the h1), `scroll={false}` for screens
 * that own their own FlatList / inbox, and `hideHeading` for pages whose
 * heading is a breadcrumb (job detail). The shell (app/portal/_layout.tsx)
 * owns the dark header, plan-state banners and tab bar.
 */
